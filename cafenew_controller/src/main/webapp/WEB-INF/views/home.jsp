...
<div class="jumbotron mt-5 bg-secondary text-white text-align: center;">
   ...
   <p id="result"></p>
   <h3>현재 지도의 표시된 카페 목록</h3>
   <div id="cafelist" class="table table-bordered table-striped"></div>
   ...
   <%--function panTo(mklatlng)
   {
      ...
      kakao.maps.event.addListener(map, 'dragend', function()
      {
         ...
         var message = '남서쪽= ' + swLatLng.getLat() + ', ' + swLatLng.getLng() + ' 입니다';
         ...
         */
         ...
         $.ajax(
                 {
                    url : "https://dapi.kakao.com/v2/local/search/category.json?category\_group\_code=CE7&rect=" + swg + "," + swt + ","+ neg + ","+ net,
                    headers :
                    {
                               ...
                               $.each(data.documents, function()
                                     {
                                           table += "<tr>"
                                              +  "<td>" + (ti + 1) + "</td>"
                                              +  "<td>" + data.documents[ti].place_name + "</td>"
                                              +  "<td>" + data.documents[ti].x + "</td>"
                                              +  "<td>" + data.documents[ti].y + "</td>"
                                              +  "<td>" + data.documents[ti].road_address_name + "</td>"
                                              +  "<td>" + data.documents[ti].address_name + "</td>"
                                              +  "<td>" +"X"+ "</td>"
                                              +  "</tr>";
                                         ti = ti + 1
                                      });
      ...
      /* function nowcafelist(data)
      {
         var tablediv = document.querySelector('#cafelist');
         console.log(data);
         html= '<table>';
          html += '<tr><th>부서번호</th><th>이름</th><th>부서</th></tr>';
          for (var i = 0; i < data.length; i++) {
              console.log("124");
              html += '<tr><td>' + data.document[i].x + '</td><td>' + data[i].y
                      + '</td><td> ' + data[i].address_name + '</td></tr>';
          }
          html += '</table>';
